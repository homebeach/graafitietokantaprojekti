...
public class DataGeneratorThreadSequentialInvoices extends Thread {
    ...
    public DataGeneratorThreadSequentialInvoices(int threadindex, int batchExecuteValue, HashMap<String, String[]> sql_databases, ReentrantLock lock, int sequentialInvoiceCount, int customerIndex, int invoiceIndex, int firstInvoiceIndex) {
    ...
    public void insertSequentialInvoices(int batchExecuteValue, Session session, List<HashMap> preparedStatementsList) throws SQLException, InterruptedException {
        ...
        int i = 0;
        while (i < sequentialInvoiceCount) {
